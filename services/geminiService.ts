...
const SYSTEM_INSTRUCTION = [
  ...
  'Earth, Fire, Water, and Air are DESIGN LANGUAGES — they translate into materiality, atmosphere, form, contrast, softness, openness, and lighting behavior. They are NEVER literal (no flames, no water waves, no wind, no soil). When the brief gives clearly separated percentages, the largest share leads the main spatial story, the next harmonizes, and smaller shares are calibrated layers. When percentages are nearly balanced (similar values across all four), treat it as a deliberate HARMONIOUS QUAD: each language gets proportional visual and atmospheric presence across zones and layers — not four chaotic styles and not a hidden single dominant.',
  ...
  'When the user prompt states elemental percentages, treat them as exact client intent for ATMOSPHERIC AND MATERIAL WEIGHT. Match the render to those shares: higher % = larger readable zones, stronger character, more lighting influence; lower % = honest calibrated traces, never erased. Near-equal percentages mean balanced multi-way fusion — intentional, coordinated, proportionally legible — not "pick one winner" and not four unrelated moods fighting.',
  '',
